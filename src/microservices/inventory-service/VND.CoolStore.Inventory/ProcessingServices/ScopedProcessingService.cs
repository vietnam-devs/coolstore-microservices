...
namespace VND.CoolStore.Inventory.ProcessingServices
{
    public class ScopedProcessingService : ScopedProcessingServiceBase, IScopedProcessingService
    {
        ...
        public async Task DoWork(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                await PublishEventsUnProcessInOutboxToChannels("inventory_service");
        ...
        {
            return assembly.GetName().Name.Contains("VND.CoolStore.Inventory.DataContracts");
        }
    }
}
