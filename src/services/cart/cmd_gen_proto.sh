...
readonly CART_PROTO_FILE=cart.proto
readonly CATALOG_PROTO_FILE=catalog.proto

cd `$SERVICE_DIR`

$GRPC_PATH/protoc -I$PROTO_PATH -I/usr/local/include -I$GOOGLEAPIS_DIR \
    --csharp_out $OUTPUT_PATH \
    --grpc_out $OUTPUT_PATH $PROTO_PATH/$CART_PROTO_FILE \
    --plugin=protoc-gen-grpc=${GRPC_PATH}/grpc_csharp_plugin

$GRPC_PATH/protoc -I$PROTO_PATH -I/usr/local/include -I$GOOGLEAPIS_DIR \
    --csharp_out $OUTPUT_PATH \
    --grpc_out $OUTPUT_PATH $PROTO_PATH/$CATALOG_PROTO_FILE \
    --plugin=protoc-gen-grpc=${GRPC_PATH}/grpc_csharp_plugin
