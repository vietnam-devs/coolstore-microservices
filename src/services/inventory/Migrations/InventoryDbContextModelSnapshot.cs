// <auto-generated />

using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using VND.CoolStore.Services.Inventory.v1.Db;

namespace VND.CoolStore.Services.Inventory.Migrations
{
    [DbContext(typeof(InventoryDbContext))]
    internal class InventoryDbContextModelSnapshot : ModelSnapshot
    {
        protected override void BuildModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.2.0-rtm-35687")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("VND.CoolStore.Services.Inventory.Domain.Inventory", b =>
            {
                b.Property<Guid>("Id")
                    .ValueGeneratedOnAdd();

                b.Property<DateTime>("Created");

                b.Property<string>("Link");

                b.Property<string>("Location");

                b.Property<int>("Quantity");

                b.Property<DateTime>("Updated");

                b.Property<int>("Version");

                b.HasKey("Id");

                b.ToTable("Inventories");

                b.HasData(
                    new
                    {
                        Id = new Guid("25e6ba6e-fddb-401d-99b2-33ddc9f29322"),
                        Created = new DateTime(2018, 12, 19, 3, 26, 34, 966, DateTimeKind.Utc).AddTicks(9388),
                        Link = "http://nashtechglobal.com",
                        Location = "London, UK",
                        Quantity = 100,
                        Updated = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                        Version = 0
                    },
                    new
                    {
                        Id = new Guid("cab3818f-e459-412f-972f-d4b2d36aa735"),
                        Created = new DateTime(2018, 12, 19, 3, 26, 34, 967, DateTimeKind.Utc).AddTicks(1464),
                        Link = "http://nashtechvietnam.com",
                        Location = "Ho Chi Minh City, Vietnam",
                        Quantity = 1000,
                        Updated = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                        Version = 0
                    });
            });
#pragma warning restore 612, 618
        }
    }
}
